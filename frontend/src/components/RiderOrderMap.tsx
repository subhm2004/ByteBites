...
const RiderOrderMap = ({ order }: Props) => {
  ...
  const deliveryLocation: [number, number] | null = hasDeliveryCoords
    ? [order.deliveryAddress.latitude, order.deliveryAddress.longitude]
    : null;

  useEffect(() => {
    if (!hasDeliveryCoords) return;
  ...
  }, [order.userId, hasDeliveryCoords]);

  if (!hasDeliveryCoords || !deliveryLocation || !riderLocation) return null;
